...
public class DevOpsRepository : IDevOpsRepository
{
    ...
    public async Task<IEnumerable<WorkItemInfo>> GetAllWorkItemInfosAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            string[] workItemTypes = ["Feature" , "Bug", "Requirement" ];
            ...
            foreach (var workItemType in workItemTypes)
            {
                var query = $@"
                    ...
                    AND [System.AreaPath] UNDER 'smartpoint\BBRZ\eLearning'
                ...
                _logger.LogInformation("Found {Count} work items of type {WorkItemType}", queryResult?.WorkItems?.Length, workItemType);

                if (queryResult?.WorkItems?.Any() == true)
                {
                    //var workItems = queryResult.WorkItems.Take(250);
                    var workItems = queryResult.WorkItems;

                    // Get work item details in batches to avoid exceeding API limits
                    const int batchSize = 100;
                    for (var i = 0; i < workItems.Count(); i += batchSize)
                    {
                        var batchIds = workItems
                            .Skip(i)
                            .Take(batchSize)
                            .Select(wi => wi.Id)
                            .ToArray();

                        _logger.LogInformation("Retrieving details for work items {Ids}", string.Join(", ", batchIds));

                        var detailsUrl = $"{_options.OrganizationUrl}/{_options.Project}/_apis/wit/workitems?ids={string.Join(",", batchIds)}&fields=System.Id,System.WorkItemType,System.ChangedDate,System.AssignedTo&api-version=7.0";
    ...
    public async Task<WorkItemDetails> LoadWorkItemByIdAsync(int workItemId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var detailsUrl = $"{_options.OrganizationUrl}/{_options.Project}/_apis/wit/workitems/{workItemId}?fields=System.Id,System.WorkItemType,System.ChangedDate,System.AssignedTo,System.Title,System.Description&api-version=7.0";
            var detailsResponse = await _httpClient.GetAsync(detailsUrl, cancellationToken);
            detailsResponse.EnsureSuccessStatusCode();
            ...
            var commentsUrl = $"{_options.OrganizationUrl}/{_options.Project}/_apis/wit/workitems/{workItemId}/comments?api-version=7.0-preview";
